...
function template_main()
{
	...
	foreach ($context['columns'] as $column)
	{
		...
		if (isset($context['old_search']))
			echo '
					<th scope="col" class="hidden-sm hidden-xs ', isset($column['class']) ? ' ' . $column['class'] : '', '"', isset($column['width']) ? ' width="' . $column['width'] . '"' : '', isset($column['colspan']) ? ' colspan="' . $column['colspan'] . '"' : '', '>
						', $column['label'], '</th>';
		// This is a selected column, so underline it or some such.
		elseif ($column['selected'])
			echo '
					<th scope="col" class="hidden-sm hidden-xs ', isset($column['class']) ? ' ' . $column['class'] : '', '" style="width: auto;"' . (isset($column['colspan']) ? ' colspan="' . $column['colspan'] . '"' : '') . ' nowrap="nowrap">
	...
	if (!empty($context['members']))
	{
		...
		{
			echo '
				<tr ', empty($member['sort_letter']) ? '' : ' id="letter' . $member['sort_letter'] . '"', ' class="member-table">
					<td class="windowbg2">
					...
					<td class="windowbg2 lefttext">', $member['link'], '</td>
		...
		if (!isset($context['disabled_fields']['website']))
			echo '
					<td class="windowbg2 hidden-sm hidden-xs">', $member['website']['url'] != '' ? '<a href="' . $member['website']['url'] . '" target="_blank" class="new_win"><img src="' . $settings['images_url'] . '/www.gif" alt="' . $member['website']['title'] . '" title="' . $member['website']['title'] . '" /></a>' : '', '</td>';

		// ICQ?
		if (!isset($context['disabled_fields']['icq']))
			echo '
					<td class="windowbg2 hidden-sm hidden-xs">', $member['icq']['link'], '</td>';

		// AIM?
		if (!isset($context['disabled_fields']['aim']))
			echo '
					<td class="windowbg2 hidden-sm hidden-xs">', $member['aim']['link'], '</td>';

		// YIM?
		if (!isset($context['disabled_fields']['yim']))
			echo '
					<td class="windowbg2 hidden-sm hidden-xs">', $member['yim']['link'], '</td>';

		// MSN?
		if (!isset($context['disabled_fields']['msn']))
			echo '
					<td class="windowbg2 hidden-sm hidden-xs">', $member['msn']['link'], '</td>';

		// Group and date.
		echo '
					<td class="windowbg2 lefttext">', empty($member['group']) ? $member['post_group'] : $member['group'], '</td>
					<td class="windowbg2 lefttext hidden-sm hidden-xs">', $member['registered_date'], '</td>';

		if (!isset($context['disabled_fields']['posts']))
		{
			echo '
					<td class="windowbg2" style="white-space: nowrap" width="15">', $member['posts'], '</td>
					<td class="windowbg2 statsbar hidden-sm hidden-xs" width="120">';
	...
	// No members?
		...
		echo '
				<tr>
					<td colspan="', $context['colspan'], '" class="windowbg2">', $txt['search_no_results'], '</td>
				</tr>';
