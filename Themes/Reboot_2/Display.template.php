...
function template_main()
{
	...
	echo '
			<div class="container-fluid id="forumposts">
				<div class="col-md-12 topic-title">
					<h2>', $txt['topic'], ': ', $context['subject'], ' &nbsp;(', $txt['read'], ' ', $context['num_views'], ' ', $txt['times'], ')</h2>
				</div>';
	...
	while ($message = $context['get_message']())
	{
		...
		echo '
				<div class="', $message['approved'] ? ($message['alternate'] == 0 ? 'windowbg-first' : 'windowbg-second') : 'approvebg', '">';
		...
		echo '
						...
						// poster info on small screens
						echo '<div class="row hidden-lg hidden-md well-sm">';
						echo '<div class="col-sm-3 col-xs-3">';
						// avatar
						// Show avatars, images, etc.?
						if (!empty($settings['show_user_images']) && empty($options['show_no_avatars']) && !empty($message['member']['avatar']['image'])){
							echo '<a href="', $scripturl, '?action=profile;u=', $message['member']['id'], '">
										', str_replace("class=\"avatar\"", "class=\"img-responsive\"", $message['member']['avatar']['image']), '
									</a>';
						}
						echo '</div>';
						echo '<div class="col-sm-9 col-xs-9">';
						//echo '<h4>';
							...
							echo '<p><strong>'. $message['member']['link'].'</strong>';
							if (!$message['member']['is_guest'])
							{
								if (!empty($message['member']['group'])){
									echo '&nbsp;<span class="label label-default">', $message['member']['group'], '</span></p>';
								}
							}
							//echo '</h4>';
							// Show the member's custom title, if they have one.
							//if (!empty($message['member']['title'])){
								//echo $message['member']['title'];
							//}
							// Don't show these things for guests.
							if (!$message['member']['is_guest'])
							{
								//if (!empty($message['member']['group'])){
									//echo '<span class="label label-default">', $message['member']['group'], '</span>';
								//}
								...
								if ($modSettings['karmaMode'] == '1'){
									echo '&nbsp;', $modSettings['karmaLabel'], ' ', $message['member']['karma']['good'] - $message['member']['karma']['bad'];
								} elseif ($modSettings['karmaMode'] == '2') 
								{
									echo '&nbsp;<p class="small">', $modSettings['karmaLabel'], ' +', $message['member']['karma']['good'], '/-', $message['member']['karma']['bad'];
						...
						echo '</div></div>';
						// poster info for large screens
						echo '<div class="poster-content hidden-sm hidden-xs">
							<h4>';
		...
		echo '
						...
						<div class="col-md-10 col-sm-12 col-xs-12 post-content">
		...
		if ($message['can_approve'])
			echo '
									<li><a href="', $scripturl, '?action=moderate;area=postmod;sa=approve;topic=', $context['current_topic'], '.', $context['start'], ';msg=', $message['id'], ';', $context['session_var'], '=', $context['session_id'], '" class="btn btn-default">', $txt['approve'], '</a></li>';

		// Can they reply? Have they turned on quick reply?
		if ($context['can_quote'] && !empty($options['display_quick_reply']))
			echo '
									<li><a href="', $scripturl, '?action=post;quote=', $message['id'], ';topic=', $context['current_topic'], '.', $context['start'], ';last_msg=', $context['topic_last_message'], '" onclick="return oQuickReply.quote(', $message['id'], ');" class="btn btn-default">', $txt['quote'], '</a></li>';

		// So... quick reply is off, but they *can* reply?
		elseif ($context['can_quote'])
			echo '
									<li><a href="', $scripturl, '?action=post;quote=', $message['id'], ';topic=', $context['current_topic'], '.', $context['start'], ';last_msg=', $context['topic_last_message'], '" class="btn btn-default">', $txt['quote'], '</a></li>';

		// Can the user modify the contents of this post?
		if ($message['can_modify'])
			echo '
									<li><a href="', $scripturl, '?action=post;msg=', $message['id'], ';topic=', $context['current_topic'], '.', $context['start'], '" class="btn btn-default">', $txt['modify'], '</a></li>';

		// How about... even... remove it entirely?!
		if ($message['can_remove'])
			echo '
									<li><a href="', $scripturl, '?action=deletemsg;topic=', $context['current_topic'], '.', $context['start'], ';msg=', $message['id'], ';', $context['session_var'], '=', $context['session_id'], '" onclick="return confirm(\'', $txt['remove_message'], '?\');" class="btn btn-default">', $txt['remove'], '</a></li>';

		// What about splitting it off the rest of the topic?
		if ($context['can_split'] && !empty($context['real_num_replies']))
			echo '
									<li><a href="', $scripturl, '?action=splittopics;topic=', $context['current_topic'], '.0;at=', $message['id'], '" class="btn btn-default">', $txt['split'], '</a></li>';

		// Can we restore topics?
		if ($context['can_restore_msg'])
			echo '
									<li><a href="', $scripturl, '?action=restoretopic;msgs=', $message['id'], ';', $context['session_var'], '=', $context['session_id'], '" class="btn btn-default">', $txt['restore_message'], '</a></li>';
		...
		echo '
							...
							<div class="moderatorbar hidden-sm hidden-xs">
	...
	echo '
			<div id="container-fluid moderationbuttons">', template_button_strip($mod_buttons, 'bottom', array('id' => 'moderationbuttons_strip')), '</div>';

	// Show the jumpto box, or actually...let Javascript do it.
	echo '
			<div class="container-fluid plainbox hidden-sm hidden-xs" id="display_jump_to">&nbsp;</div>';

	if ($context['can_reply'] && !empty($options['display_quick_reply']))
	{
		echo '
			<a id="quickreply"></a>
			<div class="container-fluid tborder" id="quickreplybox">
				<div class="cat_bar">
					<h3 class="catbg">
